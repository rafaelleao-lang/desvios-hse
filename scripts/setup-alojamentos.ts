/**
 * Cria as tabelas do módulo Alojamentos no banco desvios (principal).
 * Rodar uma única vez:
 *   npx tsx scripts/setup-alojamentos.ts
 */
...
async function main() {
  ...
  const sql = fs.readFileSync(
    path.join(__dirname, '..', 'database', 'mysql', 'migrations', '009_alojamentos.sql'),
    'utf-8',
  )
