...
function itensIniciais(): ItemFormState[] {
  return ALOJAMENTO_ITENS_CONFIG.map((cfg, i) => ({
    item_key: cfg.key,
...
interface AlojamentoFormProps {
  /** Quando informado, o formulário carrega o relatório existente e salva via update (modo edição). */
  alojamentoId?: string
}

export default function AlojamentoForm({ alojamentoId }: AlojamentoFormProps) {
  const router = useRouter()
  const { obras } = useApp()
  const isEdit = Boolean(alojamentoId)
  ...
  const [itens, setItens] = useState<ItemFormState[]>(itensIniciais)
  ...
  // Modo edição: carrega o relatório existente e popula todo o formulário.
  useEffect(() => {
    if (!alojamentoId) return
    let active = true
    alojamentosDB.find(alojamentoId)
      .then(reg => {
        if (!active || !reg) return
        setObraId(reg.obra_id)
        setAlojamentoLocalId(reg.alojamento_local_id ?? '')
        setEmpresaResponsavel(reg.empresa_responsavel)
        setNumQuartos(reg.num_quartos != null ? String(reg.num_quartos) : '')
        setNumBanheiros(reg.num_banheiros != null ? String(reg.num_banheiros) : '')
        setNumAlojados(reg.num_alojados != null ? String(reg.num_alojados) : '')
        setCapacidadeMaxima(reg.capacidade_maxima != null ? String(reg.capacidade_maxima) : '')
        setResponsavelCompra(reg.responsavel_compra ?? '')
        setResponsavelAlojamento(reg.responsavel_alojamento ?? '')
        setResponsavelRelatorio(reg.responsavel_relatorio)
        setDataVistoria(reg.data_vistoria.split('T')[0])
        setPrazoResolucao(reg.prazo_resolucao ? reg.prazo_resolucao.split('T')[0] : '')

        setItens(ALOJAMENTO_ITENS_CONFIG.map((cfg, i) => {
          const salvo = reg.itens.find(it => it.item_key === cfg.key)
          const temSubUnidades = Boolean(SUB_UNIDADE_LABELS[cfg.key])
          return {
            item_key: cfg.key,
            conforme: salvo?.conforme ?? false,
            observacao: salvo?.observacao ?? '',
            fotos: salvo?.fotos ?? [],
            uploading: false,
            aberto: i === 0,
            subUnidades: temSubUnidades
              ? (salvo?.sub_unidades && salvo.sub_unidades.length > 0
                ? salvo.sub_unidades.map(su => ({ numero: su.numero, fotos: su.fotos, observacao: su.observacao ?? '', uploading: false }))
                : [novaSubUnidade(1)])
              : undefined,
          }
        }))
  ...
  async function handleSave() {
    ...
    try {
      ...
      const itensArray = itens.map((it, i) => ({
      ...
      const result = isEdit
        ? await alojamentosDB.update(alojamentoId!, dadosCabecalho, itensArray)
        : await alojamentosDB.create(dadosCabecalho, itensArray)
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 pb-16">
      ...
      <div>
        ...
        <div className="rounded-2xl overflow-hidden">
          <div className="flex items-center gap-3 px-5 py-4" style={{ background: MSE_RED }}>
            ...
            <div>
              <p className="text-sm font-bold text-white">{isEdit ? 'Editar Relatório de Alojamento' : 'Relatório de Alojamento'}</p>
      ...
      >
        {saving
          ? <><Loader2 className="w-5 h-5 animate-spin" />{isEdit ? 'Salvando revisão…' : 'Salvando relatório…'}</>
          : <><CheckCircle2 className="w-5 h-5" />{isEdit ? 'Salvar Revisão' : 'Salvar Relatório'}</>
        }
      </button>
