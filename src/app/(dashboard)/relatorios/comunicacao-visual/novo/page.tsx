...
function drawCover(
  ...
  doc.text('RELATÓRIO', 18, 32)

  // Left: "COMUNICAÇÃO" / "VISUAL" big title (2 lines)
  doc.setFont('helvetica', 'black')
  doc.setFontSize(30)
  doc.setTextColor(255, 255, 255)
  doc.text('COMUNICAÇÃO', 18, 56)
  doc.setTextColor(...h2r(MSE_RED))
  doc.text('VISUAL', 18, 86)

  // Red accent line
  doc.setFillColor(...h2r(MSE_RED))
  doc.rect(18, 94, 40, 2.5, 'F')
  ...
  if (cityLine) doc.text(cityLine, 18, 116)

  // Elementos de Comunicação Visual — colored tags at bottom left
  const elementos = [
    { label: 'Sinalização',    cor: '#E8291C' },
    { label: 'Banners',        cor: '#F97316' },
    { label: 'Placas',         cor: '#1E3A8A' },
    { label: 'Identificação',  cor: '#0EA5E9' },
  ]
  const blockH = 8, startX = 18, startY = H - 28
  let cursorX = startX
  elementos.forEach((el) => {
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(6)
    const textW = doc.getTextWidth(el.label)
    const blockW = textW + 8
    doc.setFillColor(...h2r(el.cor))
    doc.roundedRect(cursorX, startY, blockW, blockH, 2, 2, 'F')
    doc.setTextColor(255, 255, 255)
    doc.text(el.label, cursorX + blockW / 2, startY + 5, { align: 'center' })
    cursorX += blockW + 3
  })
...
function drawDivisor(doc: any, local: string) {
  const W = 297, H = 210
  ...
  doc.setFontSize(52)
  doc.setTextColor(255, 255, 255)
  const lines = doc.splitTextToSize(local.toUpperCase(), W - 60)
  doc.text(lines, W / 2, H / 2 - (lines.length - 1) * 14, { align: 'center' })
...
function drawPhotoPage(
  ...
  // Header: "Comunicação Visual" title
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(13)
  doc.setTextColor(30, 30, 50)
  doc.text('Comunicação Visual', padX, 15)
  ...
  doc.text('Local: ', padX, 27)
  const labelW = doc.getTextWidth('Local: ')
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(9.5)
  doc.setTextColor(40, 40, 60)
  doc.text(local, padX + labelW, 27)
  ...
  doc.text('MSE Engenharia · Gestão HSE · Comunicação Visual', padX, H - 2.5)
}
...
async function gerarPDF(form: ReportFormState, obra: Obra | undefined, tst: TST | undefined) {
  ...
  for (const secao of secoesComImgs) {
    ...
    doc.addPage()
    drawDivisor(doc, secao.local)

    // 2 photos per page
    for (let i = 0; i < secao.imgs.length; i += 2) {
      const chunk = secao.imgs.slice(i, i + 2)
      doc.addPage()
      drawPhotoPage(doc, secao.local, chunk, mseLogoImg)
    }
  }
  ...
  doc.save(`Relatório Comunicação Visual - ${obraName} - Semana ${form.semana || 'S'}.pdf`)
}
    ...
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 space-y-4">
      ...
      <div>
        ...
        <input
          ...
          placeholder="Ex: Canteiro MSE, Sinalização FFEX (Onsite), Banners (Off Site)..."
...
function Step2({
  ...
  const addSecao = () => {
    setForm(f => ({
      ...
      secoes: [
        ...f.secoes,
        { id: crypto.randomUUID(), local: '', fotos: [] },
      ],
    }))
  }
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-zinc-400">
          {form.secoes.length === 0
            ? 'Adicione pelo menos um local com fotos.'
            : `${form.secoes.length} local${form.secoes.length > 1 ? 'is' : ''} · ${form.secoes.reduce((a, s) => a + s.fotos.length, 0)} fotos`}
        </p>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          {[
            ...
            { label: 'Locais',       value: String(form.secoes.length) },
      ...
      {form.secoes.length > 0 && (
        <div className="space-y-2">
          <p className="text-xs font-semibold text-zinc-500 uppercase tracking-wider">Locais</p>
          {form.secoes.map((s, i) => (
...
export default function NovoRelatorioComunicacaoVisualPage() {
    ...
    <div className="min-h-screen bg-zinc-950 p-6 lg:p-8">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-2xl font-black text-white">Comunicação <span className="text-[#E8291C]">Visual</span></h1>
